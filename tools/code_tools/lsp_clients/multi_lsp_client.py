...
class MultilspyClient:
    def __init__(self, workdir: str, project_lang: LanguageType):
        self.config = MultilspyConfig.from_dict({"code_language": project_lang})
    ...
    async def request_completions(self, file_path: str, lineno: int, charpos: int) -> list[dict]:
        with self.lsp.start_server():
    ...
    async def request_document_symbols(self, file_path: str) -> list[dict]:
        with self.lsp.start_server():
            result = await self.lsp.request_document_symbols(file_path)
        return result
    
    async def request_hover(self, file_path: str, lineno: int, charpos: int) -> list[dict]:
        with self.lsp.start_server():
            result = await self.lsp.request_hover(file_path, lineno, charpos)
        return result
    
    
    async def request_workspace_symbols(self, symbol) -> list[dict]:

        # first grep the symbol to find the file path
 
        # Execute `find` command to recursively list files and directories
        cmd = f"grep --binary-files=without-match -rnw {self.project_root} -e  {symbol}"
        ...
        with self.lsp.start_server():
            for file_path in filtered_file:
                ...
                file_symbols_dict[file_path] = result

        res_symbols = []
        for file_path, symbol_list in file_symbols_dict.items():
            
            for symbol_dict in symbol_list:
                if symbol_dict["name"] == symbol:
                    res_symbols.append((file_path, symbol_dict['selectionRange']['start']['line'], symbol_dict['selectionRange']['end']['character']))

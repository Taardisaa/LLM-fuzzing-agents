...
from ossfuzz_gen import introspector
...
class ContextRetriever:
  ...
  def _get_files_to_include(self) -> list[str]:
    ...
    for current_type in types:
      ...
      for info in info_list:
        ...
        if not include_base.endswith(('.h', '.hxx', '.hpp')):
          logging.warning(
  ...
  def get_prefixed_header_file(self, func_sig: str = '') -> Optional[str]:
    ...
    return os.path.normpath(header_file)
...
if __name__ == '__main__':
    # Example usage
    benchmark_file = "/home/yk/code/LLM-reasoning-agents/benchmark-sets/ntu/civetweb.yaml"
    benchmark = benchmarklib.Benchmark.from_yaml(benchmark_file)
       
    retriever = ContextRetriever(benchmark[0])
    # context_info = retriever.get_context_info()
    files = retriever._get_files_to_include() # type: ignore
    decl = retriever._get_embeddable_declaration() # type: ignore
    header = retriever.get_prefixed_header_file()

    print("Files to include:", files)
    ...
    error_msg = Path("/home/yk/code/LLM-reasoning-agents/outputs_ablation/gpt4/example/rank/project/civetweb/mg_get_response/run2_evcouzkkenqqknjp/build_error_4.log").read_text()
    harness_code = Path("/home/yk/code/LLM-reasoning-agents/outputs_ablation/gpt4/example/rank/project/civetweb/mg_get_response/run2_evcouzkkenqqknjp/draft_fix4.txt").read_text()
    context = oss_fuzz_code_fixer._collect_context(benchmark[0], error_msg.splitlines()) # type: ignore
    instruction = oss_fuzz_code_fixer._collect_instructions(benchmark[0], error_msg.splitlines(), harness_code) # type: ignore

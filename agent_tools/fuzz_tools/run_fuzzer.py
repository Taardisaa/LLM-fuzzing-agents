    ...
    def run_fuzzing(self, counter: int, fuzzer_name: str, ignore_crashes: bool=False, no_log: bool=False) -> tuple[ValResult, list[str], list[list[str]]]:
      ...
      # Define the error patterns
        ...
        try:
            ...
            # Wait for timeout
            while time.time() - start_time < self.run_timeout:
        ...
        '''
            Runs the fuzzer for a long time (several hours).
            Get into the docker container and run the fuzzer manually is recommended for better control.
        '''
